...
export async function getAllWorkOrders(
    ...
    try {
        const workOrders = await WorkOrder.find().sort({ _id: 1 });
...
export async function getWorkOrderByID(
    ...
    try {
        ...
        const workOrder = WorkOrder.findOne({ _id: id });
        if (!workOrder) 
...
export async function upsertWorkOrderByID(
    ...
    try {
        const id = req.params.id;
        const existing = await WorkOrder.findOne({ _id: id });
...
export async function deleteWorkOrderByID(
    ...
    try {
        const id = req.params.id;
        const deleted = await WorkOrder.findOneAndDelete({ _id: id });
